...
## loclahost:5000/
@app.route("/") # @app.route() -> localhost:5000/ 요청이 있는 경우에 바로 아래에 있는 함수를 실행한다.
def index():
...
def dashboard():
    ...
    _de_data = data["일일사망자"].tolist()

    return render_template("dashboard.html",cnt = _cnt,date_list=_date_list,decide_data=_decide_data,de_data = _de_data)
...
# request ={key:value, key2:value2, args: {ID : input에서 입력한 값(ID),password : input에서 입력한 값(password)}}
